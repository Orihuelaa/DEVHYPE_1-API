...
const Productos = () => {
  ...
  useEffect(() => {
    const fetchArticulos = async () => {
      ...
      if (articulosFromStorage) {
        ...
        try {
          const response = await articuloService.getArticulosPagedBySucursal(sucursalActiva!.id);
  ...
  const productosPaginaActual = articulos.slice(
    (paginaActual - 1) * articulosPorPagina, // Indice inicial
    paginaActual * articulosPorPagina         // Indice final
  );

  const totalPaginas = Math.ceil(articulos.length / articulosPorPagina); // Total de paginas
